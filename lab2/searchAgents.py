...
def foodHeuristic(state, problem):
    ...
    for i in range(foodLen):
        dis = mazeDistance(
            state[0], state[1].asList()[i], problem.heuristicInfo["state"]
        )
        if dis > min_dis:
            min_dis = dis
    return min_dis
        ...
        while True:
            ...
            for child in problem.getSuccessors(node):
                if child not in walls:
                    if child[0] not in closed.list and child[0] not in [
                        item[0] for item in fringe.list
                    ]:
